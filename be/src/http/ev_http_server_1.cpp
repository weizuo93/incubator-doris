...
/*http请求结束，释放request请求，删除request，在on_header()结束时被调用*/
...
static void on_request(struct evhttp_request* ev_req, void* arg) {
    ...
    request->handler()->handle(request); // 执行handler（Action）对象的handle()
}
...
/*设置收到http header时的回调函数*/
static int on_connection(struct evhttp_request* req, void* param) {
    evhttp_request_set_header_cb(req, on_header); // 设置收到http header时的回调函数
...
void EvHttpServer::start() {
    ...
    ThreadPoolBuilder("EvHttpServer")
            ...
            .build(&_workers); // 创建处理http请求的线程池

    evthread_use_pthreads();
    _event_bases.resize(_num_workers);
    for (int i = 0; i < _num_workers; ++i) { // 依次对线程池中的每一个线程进行配置
...
bool EvHttpServer::register_handler(const HttpMethod& method, const std::string& path,
    ...
    switch (method) { // http method不同，root指向不同的PathTrie<HttpHandler*>
    case GET:
        root = &_get_handlers;
        break;
    case PUT:
    ...
    if (result) {
        result = root->insert(path, handler); // 向对应method的PathTrie<HttpHandler*>中增加path对应的action（Action为HttpHandler的子类）
    }
...
int EvHttpServer::on_header(struct evhttp_request* ev_req) {
    ...
    auto handler = _find_handler(request.get()); // 寻找http请求对应的handler（Action）
    if (handler == nullptr) {
    ...
    request->set_handler(handler); // 为当前的http请求设置handler（Action）
    res = handler->on_header(request.get()); // 执行handler（Action）对象的on_header()
    if (res < 0) {
    ...
    evhttp_request_set_on_free_cb(ev_req, on_free, request.release()); // http请求结束，执行on_free()函数，释放http请求
    return 0;
}

/*寻找参数传入的HttpRequest对象对应的handler（Action）*/
HttpHandler* EvHttpServer::_find_handler(HttpRequest* req) {
    auto& path = req->raw_path(); // 获取http路径

    HttpHandler* handler = nullptr;

    std::lock_guard<std::mutex> lock(_handler_lock);
    switch (req->method()) { // 获取http method，并在对应的method下寻找当前http路径对应的handler（Action）
    case GET:
    ...
    return handler; // 返回获取到的HttpHandler对象
}

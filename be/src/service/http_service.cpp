...
Status HttpService::start() {
    add_default_path_handlers(_web_page_handler.get(), _env->process_mem_tracker());//请求web page的服务注册和响应处理
    ...
    TabletsInfoAction* tablets_info_action = new TabletsInfoAction(_env);//TabletsInfoAction类在http/action/目录下定义
    _ev_http_server->register_handler(HttpMethod::GET, "/tablets_json", tablets_info_action);//当web发送host:port/tablets_json请求时，执行TabletsInfoAction类的handle()函数
    /*******************************************************************************************/

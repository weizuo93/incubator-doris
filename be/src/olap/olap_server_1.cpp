...
Status StorageEngine::start_bg_threads() {
    RETURN_IF_ERROR(
        Thread::create("StorageEngine", "unused_rowset_monitor_thread",
                       ...
                       &_unused_rowset_monitor_thread));//创建并启动不可用rowset的监控线程
    ...
    RETURN_IF_ERROR(
        Thread::create("StorageEngine", "garbage_sweeper_thread",
                       ...
                       &_garbage_sweeper_thread));//创建并启动垃圾清除线程
    ...
    RETURN_IF_ERROR(
        Thread::create("StorageEngine", "disk_stat_monitor_thread",
                       ...
                       &_disk_stat_monitor_thread));//创建并启动磁盘状态监控线程
    ...
    if (max_compaction_concurrency < 0 || max_compaction_concurrency > base_compaction_num_threads + cumulative_compaction_num_threads + 1) {
    ...
    Compaction::init(max_compaction_concurrency);//根据最大的并发数初始化Compaction对象

    _base_compaction_threads.reserve(base_compaction_num_threads);//为vector类型的成员变量_base_compaction_threads分配空间大小
    //根据BE上能够创建的base compaction线程数量，依次创建并启动所有base compaction线程
    for (uint32_t i = 0; i < base_compaction_num_threads; ++i) {
        ...
        RETURN_IF_ERROR(
            Thread::create("StorageEngine", "base_compaction_thread",
                           ...
                           &base_compaction_thread));//创建并启动base compaction线程
        _base_compaction_threads.emplace_back(base_compaction_thread);//将启动的base compaction线程添加到成员变量_base_compaction_threads中进行管理
    }
    LOG(INFO) << "base compaction threads started. number: " << base_compaction_num_threads;

    _cumulative_compaction_threads.reserve(cumulative_compaction_num_threads);//为vector类型的成员变量_cumulative_compaction_threads分配空间大小
    //根据BE上能够创建的cumulative compaction线程数量，依次创建并启动所有cumulative compaction线程
    for (uint32_t i = 0; i < cumulative_compaction_num_threads; ++i) {
        ...
        RETURN_IF_ERROR(
                Thread::create("StorageEngine", "cumulative_compaction_thread",
                               ...
                               &cumulative_compaction_thread));//创建并启动cumulative compaction线程
        _cumulative_compaction_threads.emplace_back(cumulative_compaction_thread);//将启动的cumulative compaction线程添加到成员变量_cumulative_compaction_threads中进行管理
    }
    ...
    for (auto data_dir : data_dirs) {
        ...
        RETURN_IF_ERROR(
                Thread::create("StorageEngine", "tablet_checkpoint_thread",
                               ...
                               &tablet_checkpoint_thread));//创建并启动tablet checkpoint线程
        _tablet_checkpoint_threads.emplace_back(tablet_checkpoint_thread);//将启动的tablet checkpoint线程添加到成员变量_tablet_checkpoint_threads中进行管理
    }
    ...
    RETURN_IF_ERROR(
            Thread::create("StorageEngine", "fd_cache_clean_thread",
                           ...
                           &_fd_cache_clean_thread));//创建并启动fd cache clean线程
        ...
        for (auto data_dir : get_stores()) {
            ...
            RETURN_IF_ERROR(
                    Thread::create("StorageEngine", "path_scan_thread",
                                   ...
                                   &path_scan_thread));//创建并启动磁盘路径扫描线程
...
/*base compaction线程的回调函数*/
void StorageEngine::_base_compaction_thread_callback(DataDir* data_dir) {
#ifdef GOOGLE_PROFILER
    ProfilerRegisterThread();
#endif

    int32_t interval = config::base_compaction_check_interval_seconds; //获取base compaction检查的时间间隔配置
    do {
        if (!config::disable_auto_compaction) {
            ...
            if (!data_dir->reach_capacity_limit(0)) {
                _perform_base_compaction(data_dir);//执行base compaction
            }
        }
...
void StorageEngine::_check_cumulative_compaction_config() {
    ...
    std::string cumulative_compaction_type = config::cumulative_compaction_policy;//cumulative compaction类型分为基于大小的compaction和基于数量的compaction两种
...
void StorageEngine::_cumulative_compaction_thread_callback(DataDir* data_dir) {
    ...
    do {
        if (!config::disable_auto_compaction) {
            ...
            CgroupsMgr::apply_system_cgroup();
            if (!data_dir->reach_capacity_limit(0)) {
                _perform_cumulative_compaction(data_dir);//执行cumulative compaction
            }
        }
        ...
        //while循环正常情况下不会退出，除非有其他条件触发,wait_for()执行过程中会阻塞线程，阻塞时间长度为interval
    } while (!_stop_background_threads_latch.wait_for(MonoDelta::FromSeconds(interval)));
}

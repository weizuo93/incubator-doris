...
void StorageEngine::_perform_cumulative_compaction(DataDir* data_dir) {
    ...
    //获取最适合cumulative compaction的tablet
    ...
    CumulativeCompaction cumulative_compaction(best_tablet); //对tablet创建CumulativeCompaction对象

    OLAPStatus res = cumulative_compaction.compact();//执行cumulative compaction
    if (res != OLAP_SUCCESS) {
...
void StorageEngine::_perform_base_compaction(DataDir* data_dir) {
    ...
    BaseCompaction base_compaction(best_tablet);//对tablet创建BaseCompaction对象
    OLAPStatus res = base_compaction.compact();//执行base compaction
    if (res != OLAP_SUCCESS) {
...
/*将不可用的rowset(完成了compaction)添加到成员变量_unused_rowsets中*/

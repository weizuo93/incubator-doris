...
/*根据tablet_meta创建tablet*/
TabletSharedPtr Tablet::create_tablet_from_meta(TabletMetaSharedPtr tablet_meta, DataDir* data_dir) {
    return std::make_shared<Tablet>(tablet_meta, data_dir);//调用Tablet的构造函数，并返回一个指向Tablet对象的std::shared_ptr智能指针
}

/*构造函数。根据tablet meta创建tablet对象，创建的tablet会存储在data dir中*/
Tablet::Tablet(TabletMetaSharedPtr tablet_meta, DataDir* data_dir) :
    ...
    _rs_graph.construct_rowset_graph(_tablet_meta->all_rs_metas());//构建rowset图
}

/*初始化成员变量_rs_version_map和_inc_rs_version_map。
  这两个成员变量的类型都是std::unordered_map<Version, RowsetSharedPtr, HashOfVersion>。新的加载作业都会产生一个rowset，它将被插入到_rs_version_map和_inc_rs_version_map中，
  只有最近的rowset会被保存在_inc_rs_version_map中以减少克隆任务中需要复制的数据量。注意：并非_inc_rs_version_map中所有的rowset都在_rs_version_map中，因为一些rowset被合并之后
  就会从_rs_version_map中删除，但是也许不会从_inc_rs_version_map中删除。_inc_rs_version_map中的哪些rowset会被删除取决于参数inc_rowset_expired_sec的配置，另外，删除操作会被
  周期性触发，因此，在特定的时间点（比如刚刚完成了一次base compaction），_inc_rs_version_map中某些的rowset不在_rs_version_map中*/
OLAPStatus Tablet::_init_once_action() {
    OLAPStatus res = OLAP_SUCCESS;
    VLOG(3) << "begin to load tablet. tablet=" << full_name()
            << ", version_size=" << _tablet_meta->version_count();

    //初始化成员变量_rs_version_map
    for (const auto& rs_meta :  _tablet_meta->all_rs_metas()) {//依次获取当前tablet下的所有rowset的meta信息，tabet meta调用all_rs_metas()函数可以实现该功能。
        Version version = rs_meta->version();//获取rowset meta的版本信息
        RowsetSharedPtr rowset;
        res = RowsetFactory::create_rowset(&_schema, _tablet_path, rs_meta, &rowset);//根据rowset meta，tablet path和schema创建rowset
        if (res != OLAP_SUCCESS) {
        ...
        _rs_version_map[version] = std::move(rowset);//将创建的rowset添加到成员变量_rs_version_map中
    }

    // 初始化成员变量_inc_rs_version_map（init incremental rowset）
    for (auto& inc_rs_meta : _tablet_meta->all_inc_rs_metas()) {//依次获取当前tablet下的所有incremental rowset的meta信息，tabet meta调用all_inc_rs_metas()函数可以实现该功能。
        Version version = inc_rs_meta->version();//获取incremental rowset meta的版本信息
        RowsetSharedPtr rowset = get_rowset_by_version(version);//根据version获取rowset
        if (rowset == nullptr) {
            res = RowsetFactory::create_rowset(&_schema, _tablet_path, inc_rs_meta, &rowset);根据incremental rowset meta，tablet path和schema创建incremental rowset
            if (res != OLAP_SUCCESS) {
        ...
        _inc_rs_version_map[version] = std::move(rowset);//将创建的incremental rowset添加到成员变量_inc_rs_version_map中
    }
...
OLAPStatus Tablet::init() {
    return _init_once.call([this] { return _init_once_action(); });//初始化成员变量_rs_version_map和_inc_rs_version_map
}
...
void Tablet::save_meta() {
    auto res = _tablet_meta->save_meta(_data_dir);//保存tablet meta
...
/*向成员变量rs_version_map中添加rowset，向成员变量_rs_graph中添加rowset的版本*/
OLAPStatus Tablet::add_rowset(RowsetSharedPtr rowset, bool need_persist) {
    ...
    _rs_version_map[rowset->version()] = rowset;//向成员变量_rs_version_map中添加传入的rowset
    _rs_graph.add_version_to_graph(rowset->version());//向成员变量_rs_graph中添加传入的rowset的版本
...
/*修改tablet的rowset meta，重建rowset graph*/
void Tablet::modify_rowsets(const vector<RowsetSharedPtr>& to_add,
    ...
    //获取tablet中需要删除的rowset meta
    vector<RowsetMetaSharedPtr> rs_metas_to_delete;
    for (auto& rs : to_delete) {
        rs_metas_to_delete.push_back(rs->rowset_meta());
        _rs_version_map.erase(rs->version());//从成员变量_rs_version_map中擦除要删除的rowset 版本信息
    }
    ...
    for (auto& rs : to_add) {
        ...
        _rs_version_map[rs->version()] = rs;//向成员变量_rs_version_map中新增要添加的rowset 版本信息
        ++_newly_created_rowset_num;
    }

    _tablet_meta->modify_rs_metas(rs_metas_to_add, rs_metas_to_delete);//修改tablet的rowset meta
    _rs_graph.reconstruct_rowset_graph(_tablet_meta->all_rs_metas());//重建rowset graph
}
...
const RowsetSharedPtr Tablet::get_rowset_by_version(const Version& version) const {
    auto iter = _rs_version_map.find(version);//从成员变量_rs_version_map中查找指定版本的rowset
...
const RowsetSharedPtr Tablet::get_inc_rowset_by_version(const Version& version) const {
    auto iter = _inc_rs_version_map.find(version);//从成员变量_inc_rs_version_map中查找指定版本的incremental rowset
...
/*获取tablet中最大版本的rowset*/
// Already under _meta_lock
const RowsetSharedPtr Tablet::rowset_with_max_version() const {
    Version max_version = _tablet_meta->max_version();//通过tablet meta来获取rowset的最大版本
...
/*向tablet（成员变量_inc_rs_version_map）中添加rowset*/
...
OLAPStatus Tablet::add_inc_rowset(const RowsetSharedPtr& rowset) {
    DCHECK(rowset != nullptr);
    WriteLock wrlock(&_meta_lock);
    if (_contains_rowset(rowset->rowset_id())) {//如果rowset已经存在，则直接返回。
        return OLAP_SUCCESS;
    }
    RETURN_NOT_OK(_contains_version(rowset->version()));

    RETURN_NOT_OK(_tablet_meta->add_rs_meta(rowset->rowset_meta()));
    _rs_version_map[rowset->version()] = rowset;//向成员变量_inc_rs_version_map中添加rowset
    _inc_rs_version_map[rowset->version()] = rowset;//向成员变量_inc_rs_version_map中添加rowset
    _rs_graph.add_version_to_graph(rowset->version());//向成员变量_rs_graph中添加rowset的版本
    RETURN_NOT_OK(_tablet_meta->add_inc_rs_meta(rowset->rowset_meta()));
    ++_newly_created_rowset_num;//添加完rowset之后，新创建的rowset数量增加1
    return OLAP_SUCCESS;
}
...
void Tablet::_delete_inc_rowset_by_version(const Version& version,
    ...
    RowsetMetaSharedPtr rowset_meta = _tablet_meta->acquire_inc_rs_meta_by_version(version);//根据版本号获取rowset meta
    if (rowset_meta == nullptr) {
        return;
    }
    _tablet_meta->delete_inc_rs_meta_by_version(version);//从meta中删除对应版本rowset的meta
    VLOG(3) << "delete incremental rowset. tablet=" << full_name() << ", version=" << version;
}

/*从tablet中删除到期的rowset*/
void Tablet::delete_expired_inc_rowsets() {
    int64_t now = UnixSeconds();//获取当前时间
    vector<pair<Version, VersionHash>> expired_versions;//用于保存到期的rowset版本信息
    WriteLock wrlock(&_meta_lock);
    for (auto& rs_meta : _tablet_meta->all_inc_rs_metas()) {
        double diff = ::difftime(now, rs_meta->creation_time());//通过rowset meta获取rowset的创建时间，并计算rowset当前已经存在的时间
        if (diff >= config::inc_rowset_expired_sec) {//判断rowset是否到期
    ...
    for (auto& pair: expired_versions) {
        _delete_inc_rowset_by_version(pair.first, pair.second);//通过版本号从tablet（实质上是成员变量_inc_rs_version_map）中依次删除对应的rowset
...
/*获取tablet中所有的rowset 版本信息，通过参数传回*/
void Tablet::list_versions(vector<Version>* versions) const {
    DCHECK(versions != nullptr && versions->empty());

    versions->reserve(_rs_version_map.size());//为vector类型的变量versions分配空间大小
    // versions vector is not sorted.
    for (const auto& it : _rs_version_map) {
        versions->push_back(it.first);//向变量versions中依次添加所有的版本
    }
}
...
void Tablet::max_continuous_version_from_begining(Version* version, VersionHash* v_hash) {
    ReadLock rdlock(&_meta_lock);
...
void Tablet::delete_all_files() {
    ...
    _rs_version_map.clear();//清空vector类型的成员变量rs_version_map
    for (auto it : _inc_rs_version_map) {
        it.second->remove();
    }
    _inc_rs_version_map.clear();//清空vector类型的成员变量_inc_rs_version_map
}

/*路径检查，检查输入的路径是否与当前tablet的路径或tablet中的rowset路径一致*/
...
/*检查输入的rowset is是否在当前tablet的rowset中存在*/
...
/*输入的rowset版本是否已经在当前tablet的rowset中被包含，如果未被包含则返回OLAP_SUCCESS*/
...
/*获取compaction的状态，通过参数返回json格式的字符串结果*/

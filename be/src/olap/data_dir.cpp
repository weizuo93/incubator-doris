...
/*data dir的健康检查，判断磁盘是否可用以及读写测试文件是否成功。如果读写测试文件失败，会将磁盘标记为不可用。*/
...
OLAPStatus DataDir::_read_and_write_test_file() {
    std::string test_file = _path + kTestFilePath;//kTestFilePath被定义如下：static const char* const kTestFilePath = "/.testfile";
    return read_write_test_file(test_file);;
}

/*在磁盘上获取一个shard供创建tablet时使用，获取结果通过参数传回。*/
//下次再获取shard时，就会获取到(_current_shard + 1) % MAX_SHARD_NUM之后的另一个shard
OLAPStatus DataDir::get_shard(uint64_t* shard) {
    ...
    uint32_t next_shard = 0;
    {
        ...
        _current_shard = (_current_shard + 1) % MAX_SHARD_NUM; //成员变量_current_shard 始终保存下一次的shard
    }
...
/*在磁盘中注册tablet，本质上是将tablet的信息添加到成员变量_tablet_set中*/
...
/*在磁盘中对tablet取消注册，本质上是将tablet的信息从成员变量_tablet_set中移除*/
...
/*获取不可用磁盘上的tablet信息，并从数据结构_tablet_set中删除这些tablet数据*/
void DataDir::clear_tablets(std::vector<TabletInfo>* tablet_infos) {
    std::lock_guard<std::mutex> l(_mutex);

    tablet_infos->insert(tablet_infos->end(), _tablet_set.begin(), _tablet_set.end());//获取不可用磁盘上的tablet信息
    _tablet_set.clear();//从数据结构_tablet_set中删除这些tablet数据
}

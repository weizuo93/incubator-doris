...
OLAPStatus TabletMeta::create(const TCreateTabletReq& request, const TabletUid& tablet_uid,
    ...
    tablet_meta->reset(new TabletMeta(request.table_id, request.partition_id, //智能指针shared_ptr中的reset(p)方法含义是将智能指针重置为p的值
...
OLAPStatus TabletMeta::serialize(string* meta_binary) {
    ...
    // 为了验证序列化的结果，将序列化生成的字符串meta_binary进行反序列化，如果能够成功反序列化，说明序列化过程没有问题。deserialize the meta to check the result is correct
    TabletMetaPB de_tablet_meta_pb;
    bool parsed = de_tablet_meta_pb.ParseFromString(*meta_binary);//从字符串meta_binary中解析tablet meta信息到TabletMetaPB对象中
    if (!parsed) {
...
/*从字符串meta_binary中解析tablet meta信息*/
OLAPStatus TabletMeta::deserialize(const string& meta_binary) {
    TabletMetaPB tablet_meta_pb;
    bool parsed = tablet_meta_pb.ParseFromString(meta_binary);//从字符串meta_binary中解析tablet meta信息到TabletMetaPB对象中
    if (!parsed) {
...
/*从TabletMetaPB对象中解析出tablet meta信息*/

...
public class ConnectProcessor {
    ...
    private void handleQuery() {
        ...
        try {
            List<StatementBase> stmts = analyze(originStmt); //对原始的sql语句进行解析，得到多个解析之后生成的语法树
            for (int i = 0; i < stmts.size(); ++i) {  //循环处理sql解析生成的语法树
                ctx.getState().reset();
                if (i > 0) {
                    ctx.resetRetureRows();
                }
    ...
    private List<StatementBase> analyze(String originStmt) throws AnalysisException {
        ...
        try {
            return SqlParserUtils.getMultiStmts(parser); //返回解析之后生成的语法树
        } catch (Error e) {
